...
public class cgMoveGenerator
{
    ...
    public cgMoveGenerator(byte width = 8, byte height = 8)
    {
        ...
        int thirthRowIndex = width * 2;
        int thirthLastRowIndex = width * (height - 3);
        _whitePawnRow = new List<int>();
        _blackPawnRow = new List<int>();
        for (var u = 0; u < width; u++)
        {
            _blackPawnRow.Add(thirthRowIndex + u);
            _whitePawnRow.Add(thirthLastRowIndex + u);
        }
    ...
    /// </summary>
    /// <param name="indexPosition">The 0-81 index position</param>
    /// <returns></returns>
    ...
    /// </summary>
    /// <param name="pos">The 0-81 index position</param>
    /// <returns>All possible moves.</returns>
    ...
    {
        List<int> directions = new List<int>() { _LEFT, _LEFT_DOWN, _DOWN, _RIGHT_DOWN, _RIGHT, _RIGHT_UP, _UP, _LEFT_UP };
        List<int> queenMovement = FindRayMoves(directions, pos);
        List<int> knigthMovement = EmulateKnightAt(pos);
        List<int> knigthMovement2 = new List<int>();

        //Movimiento del caballo separado por -1
        knigthMovement2.Add(-1);

        int j = 0;
        for (int i=0; i < knigthMovement.Count*2; i++)
        {
            //SI i es par añadimos el movimiento del caballo
            if (i%2 == 0)
            {
                knigthMovement2.Add(knigthMovement[j]);
                j++;
            }
            else//Y si es impar metemos un -1
            {
                knigthMovement2.Add(-1);
            }
        ...
        queenMovement.AddRange(knigthMovement2);
    ...
    /// <param name="pos">The 0-81 index position</param>
    /// <returns>All possible moves.</returns>
    public List<int> EmulateDameAt(int pos)
    {
        List<int> directions = new List<int>() { _LEFT, _LEFT_DOWN, _DOWN, _RIGHT_DOWN, _RIGHT, _RIGHT_UP, _UP, _LEFT_UP };
        return FindRayMoves(directions, pos);
    }
