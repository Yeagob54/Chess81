...
public class cgMenuScript : MonoBehaviour {
    ...
    /// The current mode.
    /// </summary>
    public BoardMode currentBoardMode;
    ...
    {
        if (HumanAsBlack.isOn && HumanAsWhite.isOn) currentBoardMode = BoardMode.PlayerVsPlayer;
        else if (HumanAsBlack.isOn && !HumanAsWhite.isOn) currentBoardMode = BoardMode.EngineVsPlayer;
        else if (!HumanAsBlack.isOn && HumanAsWhite.isOn) currentBoardMode = BoardMode.PlayerVsEngine;
        else if (!HumanAsBlack.isOn && !HumanAsWhite.isOn) currentBoardMode = BoardMode.EngineVsEngine;

        //Change the mode display text according to the current board mode.
        if (currentBoardMode == BoardMode.PlayerVsPlayer)
        {
        ...
        {
            if (currentBoardMode == BoardMode.PlayerVsEngine) ModeDisplayText.text = "Player vs Engine";
            if (currentBoardMode == BoardMode.EngineVsPlayer) ModeDisplayText.text = "Engine vs Player";
            if (currentBoardMode == BoardMode.EngineVsEngine) ModeDisplayText.text = "Engine vs Engine";
