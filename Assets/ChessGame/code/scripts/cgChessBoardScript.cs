...
public class cgChessBoardScript : MonoBehaviour
{
    ...
    /// </summary>
    public TextMeshProUGUI moveLog;
    ...
    /// </summary>
    public Transform blackDeadPiecesLocation ;
    /// <summary>
    /// What location should dead white pieces be placed at?
    /// </summary>
    public Transform whiteDeadPiecesLocation;
    /// <summary>
    ...
    /// </summary>
    public byte searchDepthStrong = 2;
    ...
    private bool _humanPlayerIsBlack
    {
        get
        {
            return ((Mode == BoardMode.EngineVsPlayer) || (Mode == BoardMode.PlayerVsPlayer) || (Mode == BoardMode.OnlineBlack));
        }
    }
    ...
    private bool _humanPlayerIsWhite
    {
        get
        {
            return ((Mode == BoardMode.PlayerVsEngine) || (Mode == BoardMode.PlayerVsPlayer) || (Mode == BoardMode.OnlineWhite));
        }
    }
    ...
    public void placePieces()
    {
        ...
        {
            GameObject newPiece = Instantiate(chessPiecePrefab, chessPieceHolder.transform);      

            Destroy(piece.gameObject);
            pieces.Add(newPiece.GetComponent<cgChessPieceScript>());
        }
    ...
    public void FlipBoard()
    {
        if (flipTarget != null)
        {
            ...
            flipTarget.transform.localRotation = Quaternion.Euler(0, 180, 180);
            flipTarget.transform.localPosition += Vector3.up * -0.55f;
    ...
    private void _pieceDown(cgChessPieceScript piece)
    {
        if (highlightLegalMoves && playerCanMove)
        {
            ...
            foreach (cgSimpleMove move in moves)
            {
                ...
                {
                    cgSquareScript targetSquare = _getSquare(_abstractBoard.SquareNames[move.to]);
                    ...
                    //else
                    //{
                        targetSquare.changeColor(targetSquare.legalMoveToColor);
                    //}
                }
        ...
        if (_downPiece != null)
        {
            if (playerCanMove || Mode == BoardMode.PlayerVsPlayer)
            {
                ...
                foreach (cgSimpleMove move in legalMoves)
                {
                    if (move is cgCastlingMove)
                    {
                        if (_abstractBoard.SquareNames[move.from] == _downPiece.square.uniqueName && _abstractBoard.SquareNames[(move as cgCastlingMove).to] == closestSquare.uniqueName)
                        {
                            legalMove = move;
                        }
                    }
                ...
                if (legalMove != null && _abstractBoard.verifyLegality(legalMove))
                {
                    ...
                    //ENVÍO DEL MOVIMIENTO POR RPC!!!
                    if (Mode == BoardMode.OnlineBlack || Mode == BoardMode.OnlineWhite)
                    {
                        if (legalMove is cgEnPassantMove)
                        {
                            NetworkManager.instance.photonView.RPC("SendMove", NetworkManager.instance.rpcTarget, legalMove.from, legalMove.to, (legalMove as cgEnPassantMove).attackingSquare, false);
                        }
                        else
                        {
                            if (legalMove.queened)
                            {
                                NetworkManager.instance.photonView.RPC("SendMove", NetworkManager.instance.rpcTarget, legalMove.from, legalMove.to, (byte)0, true);
                            }
                            else
                            {
                                if (legalMove is cgCastlingMove)
                                    NetworkManager.instance.photonView.RPC("SendCastlingMove", NetworkManager.instance.rpcTarget, legalMove.from, legalMove.to, (legalMove as cgCastlingMove).secondFrom, (legalMove as cgCastlingMove).secondTo);
                                else
                                    NetworkManager.instance.photonView.RPC("SendMove", NetworkManager.instance.rpcTarget, legalMove.from, legalMove.to, (byte)0, false);
                    ...
                    //Si el movimiento no es legal la pieza vuelve a su casilla
    ...
    public void OnlineMovement (cgSimpleMove movement)
    {
        _makeMove(movement);
    }

    public void OnlineMovement(cgEnPassantMove movement)
    {
        _makeMove(movement);
    }
    ...
    private void _gameOver(bool whiteWins, bool blackWins)
    {
        ...
        //TODO: se podría refactorizar esto, pero si funciona tampoco hace falta...
        if (whiteWins && Mode == BoardMode.OnlineBlack) //WHITE WIS - you black loses
            GameController.instance.MatchLosed("playing");
        else if (blackWins && (Mode == BoardMode.OnlineWhite || Mode == BoardMode.PlayerVsEngine))//BLACK WIS - you white loses
            GameController.instance.MatchLosed("playing");
        else if (whiteWins && Mode == BoardMode.PlayerVsEngine)//You win vs Bot
            GameController.instance.AddMatchWin("playing");
        else
            GameController.instance.MatchDraw();//DRAW
    ...
    private void _makeMove(cgSimpleMove move)
    {
        ...
        movesMade++;
        playSound(moveSound);
        GetComponent<AudioSource>().Stop();//Stop warning sounds
    ...
    public void _makeEngineMoveMono(cgBoard board, bool moveAsWhiteP, Action<List<cgSimpleMove>> callback)
    {
        ...
        Action<float> progessCallback = (float progress) =>
        {
            ...
            {
                _engineProgress.Add(progress);
                //if (Time.realtimeSinceStartup > initialTime +30)
            }
        };
    ...
    private void _setDeadPiece(cgChessPieceScript cp)
    {
        ...
        if (!_deadPieces.Contains(cp))
        {
            ...
            if (cp.white)
            {
                cp.transform.position = whiteDeadPiecesLocation.position + new Vector3((int)(_deadWhitePieces%5) * 0.53f, -(float)((int)(_deadWhitePieces/5) * .48f), 0);
                cp.transform.localScale *= 0.65f;
            }
            else if (!cp.white)
            {
                cp.transform.position = blackDeadPiecesLocation.position + new Vector3((int)(_deadBlackPieces % 5) * -0.53f, -(float)((int)(_deadBlackPieces / 5) * .48f), 0);
                cp.transform.localScale *= 0.65f;
            }
            _deadPieces.Add(cp);
    ...
    private void _writeLog(cgSimpleMove move)
    {
        if (moveLog != null)
        {
            if (NotationType == cgNotation.NotationType.Coordinate)
            {
                ...
                string piece = _getPieceOn(_abstractBoard.SquareNames[move.from])._shortName();
                moveLog.text = _abstractBoard.moves.Count +"-" +piece + ": "+ _abstractBoard.SquareNames[move.from] + "-" + _abstractBoard.SquareNames[move.to] + moveLog.text;
            }
